...
/**
 * This class implements the ASCoDT Project nature. In eclipse one project can
 * different natures. A nature is a function,
 * which maps a project to compiler(builder). In the case of ASCoDT the ASCoDT
 * nature tells eclipse to use the internal
 * ASCoDT builder to be able to compile the project. The life-cycle of the
 * ASCoDT nature is the following:
 * 1. After load:
 * -Configure the nature: reads the project description file(.project), which
 * holds the state of the project and loads the
 * serialized parameters for the compiler(builder) and installs the compiler for
 * this project
 * 2. Destroy:
 * -remove all parameters from the project description and removes the installed
 * compilers
 * Important functionality:
 * - the nature acts as an access point to the compiler arguments. We can access
 * them through getArguments() and setArguments
 * 
 * @author atanasoa
 * 
 */
public class ASCoDTNature implements IProjectNature {
  ...
  public void configure() throws CoreException {
    ...
    for (ICommand command : commands) {
      if (command.getBuilderName()
                 .equals(de.tum.ascodt.plugin.project.builders.SiDLBuilder.ID)) {
        return;
      }
    }
   ...
   * 
   * @param args
   *          compiler arguments for the ASCoDT project
   * @throws CoreException
   */
