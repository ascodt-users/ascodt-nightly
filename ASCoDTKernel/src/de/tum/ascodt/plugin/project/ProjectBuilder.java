 ...
 * 
 * It creates the basic files of the project and add the project natures and
 * builders.
 * 
 ...
 *       - Other components can create new projects due to this manager (factory
 *       mechanism).
 *       - Other components can gain access to the project due to this manager
 *       with the
 *       project name.
 * 
...
public class ProjectBuilder extends PlatformObject {
   ...
   */
  protected Map<String, Project> _nameToProjectEntitiesMap;
  ...
  /**
   * Creates a project representation object for existing eclipse project with
   * the ASCoDT nature
   * The method is used by starting of eclipse when we have only eclipse
   * projects but no ascodt
   * projects
   * 
   * @param projectName
  ...
  public void createProject(IProject eclipseProject) throws ASCoDTException {
    ...
    notiflyAllListeners();
    newProject.resetStaticRepository();
  ...
  public Project getProject(String projectName) {
    Assert.isTrue(_nameToProjectEntitiesMap.containsKey(projectName));
    return _nameToProjectEntitiesMap.get(projectName);
  }
