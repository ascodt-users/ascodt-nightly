...
public class ProcessExitDetector implements Runnable{
	...
	public void run() {
		try {
			...
			notifyListeners(-1);
			ErrorWriterDevice.getInstance().showError( getClass().getName(), "ProcessExitDetector",  "process waitFor interrupted",e );
	...
	public synchronized void notifyListeners(int returnValue){
		try {
			...
			ErrorWriterDevice.getInstance().showError( getClass().getName(), "notifyListeners()",  "process finished error!",e );
			e.printStackTrace();
		}
	}
