...
public class BuildSIDLHandler extends AbstractHandler {
				...
				Iterator<Object> iterator = strucSelection.iterator(); iterator.hasNext();) {
					...
					if(element instanceof IResource&&((IResource) element).getProject().hasNature(ASCoDTNature.ID))
						ProjectBuilder.getInstance()
							.getProject(((IResource) element).getProject()).buildProjectSources();

				}
			}
		} catch (CoreException e) {
			ErrorWriterDevice.getInstance().showError( getClass().getName(), "execute()",  "Cannot compile SIDL", e );
