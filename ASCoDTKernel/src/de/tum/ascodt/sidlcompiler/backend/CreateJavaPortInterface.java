...
public class CreateJavaPortInterface extends DepthFirstAdapter {
  private static Trace _trace = new Trace(
      CreateJavaPortInterface.class.getCanonicalName());
    ...
    try {
      if (!_generateSuperPort) {
        ...
        _fullQualifiedComponentName = _symbolTable.getScope(node)
            .getFullQualifiedName(portName);
        String destinationFileOfInterface = _destinationDirectory.toString() +
            File.separatorChar +
            _fullQualifiedComponentName.replaceAll("[.]", "/") + ".java";
        String destinationFileOfDispatcherPort = _destinationDirectory
            .toString() +
            File.separatorChar +
            _fullQualifiedComponentName.replaceAll("[.]", "/") +
            "Dispatcher.java";
        String destinationFileOfAbstractPort = _destinationDirectory.toString() +
            File.separatorChar +
            _fullQualifiedComponentName.replaceAll("[.]", "/") +
            "AbstractPort.java";
        String destinationFileOfAbstractSocketPort = _destinationDirectory
            .toString() +
            File.separatorChar +
            _fullQualifiedComponentName.replaceAll("[.]", "/") +
            "AbstractSocketPort.java";
        String destinationFileOfSocketPort = _destinationDirectory.toString() +
            File.separatorChar +
            _fullQualifiedComponentName.replaceAll("[.]", "/") +
            "SocketPort.java";

        String destinationFileOfDispatcherNativePort = _destinationDirectory
            .toString() +
            File.separatorChar +
            _fullQualifiedComponentName.replaceAll("[.]", "/") +
            "NativeDispatcher.java";
        String destinationFileOfDispatcherSocketPort = _destinationDirectory
            .toString() +
            File.separatorChar +
            _fullQualifiedComponentName.replaceAll("[.]", "/") +
            "SocketDispatcher.java";

        _portsHashmap.put(_fullQualifiedComponentName + "createPort",
            2 + _portsHashmap.size());
        _portsHashmap.put(_fullQualifiedComponentName + "connectPort",
            2 + _portsHashmap.size());
        _portsHashmap.put(_fullQualifiedComponentName + "disconnectPort",
            2 + _portsHashmap.size());

        _templateInterfaceFiles.push(new TemplateFile(templateFileOfInterface,
            destinationFileOfInterface, _namespace, TemplateFile
                .getLanguageConfigurationForJava(), true));
        _templateDispatcherFiles.push(new TemplateFile(
            templateFileOfDispatcherPort, destinationFileOfDispatcherPort,
            _namespace, TemplateFile.getLanguageConfigurationForJava(), true));
        _templateDispatcherNativeFiles.push(new TemplateFile(
            templateFileOfDispatcherNativePort,
            destinationFileOfDispatcherNativePort, _namespace, TemplateFile
                .getLanguageConfigurationForJava(), true));
        _templateDispatcherSocketFiles.push(new TemplateFile(
            templateFileOfDispatcherSocketPort,
            destinationFileOfDispatcherSocketPort, _namespace, TemplateFile
                .getLanguageConfigurationForJava(), true));
        TemplateFile templateAbstractPort = new TemplateFile(
            templateFileOfAbstractPort, destinationFileOfAbstractPort,
            _namespace, TemplateFile.getLanguageConfigurationForJava(), true);
        TemplateFile templateAbstractSocketPort = new TemplateFile(
            templateFileOfAbstractSocketPort,
            destinationFileOfAbstractSocketPort, _namespace,
            TemplateFile.getLanguageConfigurationForJava(), true);
        TemplateFile templateSocketPort = new TemplateFile(
            templateFileOfSocketPort, destinationFileOfSocketPort, _namespace,
            TemplateFile.getLanguageConfigurationForJava(), true);
  ...
  public void inAOperation(AOperation node) {
    ...
    try {
      ...
      TemplateFile template = new TemplateFile(_templateInterfaceFiles.peek(),
          templateFile);

      String templateDispatcherOpearationFile = "java-port-dispatcher-operation.template";

      TemplateFile templateDispatcherOpertion = new TemplateFile(
          _templateDispatcherFiles.peek(), templateDispatcherOpearationFile);
