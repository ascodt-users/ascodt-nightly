...
public class CreateJNIProxyForFortran extends de.tum.ascodt.sidlcompiler.frontend.analysis.DepthFirstAdapter{
	private Trace                      _trace = new Trace(CreateJNIProxyForFortran.class.getCanonicalName());
	...
	CreateJNIProxyForFortran(SymbolTable symbolTable, URL userImplementationsDestinationDirectory
	...
	public void inAClassPackageElement(AClassPackageElement node) {
		...
		try {
			...
			_templateFilesOfFortranProxyImplementation.push(
					new TemplateFile( templateFileForAbstractFortranProxyComponentImplemention, destinationFileForFortranProxyImplementation, _namespace, TemplateFile.getLanguageConfigurationForJNI() ,true)
					);
			...
			_templateFilesOfJNIProxyImplementation.peek().addMapping( "__COMPONENT_NAME__", componentName.toLowerCase() );
