...
public class CreateNative2JavaPlainPorts extends DepthFirstAdapter {
  private static Trace _trace = new Trace(
      CreateNative2JavaPlainPorts.class.getCanonicalName());
    ...
    try {
      if (!_generateSuperport) {
        ...
        String fullQualifiedPortName = _symbolTable.getScope(node)
            .getFullyQualifiedName(portType);
        String destinationFileJava = _destinationDirectory.toString() +
            File.separatorChar + fullQualifiedPortName.replaceAll("[.]", "/") +
        ...
        _templateFilesJava.push(new TemplateFile(templateFileJava,
        ...
        _templateFilesImplementation.push(new TemplateFile(
            templateFileForImplementation, destinationFileImplementation,
  ...
  public void inAOperation(AOperation node) {
    ...
    try {
      ...
      TemplateFile templateJava = new TemplateFile(_templateFilesJava.peek(),
          templateFileJava);
      ...
      templateImplementation.addMapping("__OPERATION_PARAMETERS_LIST__",
          parameterList.getParameterListInCxx());
      templateImplementation.addMapping(
          "__JNI_OPERATION_PARAMETER_TYPES_LIST__",
