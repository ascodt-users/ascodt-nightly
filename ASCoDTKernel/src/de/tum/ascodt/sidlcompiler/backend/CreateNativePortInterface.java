...
public class CreateNativePortInterface extends DepthFirstAdapter {
  ...
  private void generateEnumIncludes() {
    for (de.tum.ascodt.sidlcompiler.frontend.node.AEnumDeclarationPackageElement globalEnumeration : _symbolTable
        .getGlobalScope().getFlattenedEnumsElements()) {
      for (String localEnumeration : _enums) {
        if (globalEnumeration
            .getName()
            .getText()
            .contains(
                localEnumeration.substring(localEnumeration.lastIndexOf(".") + 1))) {
          String fullQualifiedName = _symbolTable.getScope(globalEnumeration)
              .getFullQualifiedName(globalEnumeration.getName().getText());
          _enumerationIncludes += "#include \"" +
              _destinationDirectory.getPath().toString() +
  ...
  public void inAInterfacePackageElement(AInterfacePackageElement node) {
    ...
    try {
      ...
      String destinationFileOfInterface = _destinationDirectory.toString() +
          File.separatorChar + fullQualifiedName.replaceAll("[.]", "/") + ".h";
      ...
      String destinationFileOfSocketDispatcherPortHeader = _destinationDirectory
          .toString() +
          File.separatorChar +
          fullQualifiedName.replaceAll("[.]", "/") + "NativeSocketDispatcher.h";
      ...
      _templateFilesSocketDispatcherHeader.push(new TemplateFile(
          ...
          destinationFileOfSocketDispatcherPortHeader, _namespace, TemplateFile
              .getLanguageConfigurationForCPP(), true));
      _templateFilesSocketDispatcherImplementation.push(new TemplateFile(
          ...
          TemplateFile.getLanguageConfigurationForCPP(), true));
      _templateFilesFortranSocketDispatcher.push(new TemplateFile(
          templateFileOfFortranSocketDispatcherImplementationPort,
          destinationFileOfFortranSocketDispatcherPortImplementation,
          _namespace, TemplateFile.getLanguageConfigurationForFortran(), true));
      _templateFilesFortranProxy4SocketDispatcher.push(new TemplateFile(
