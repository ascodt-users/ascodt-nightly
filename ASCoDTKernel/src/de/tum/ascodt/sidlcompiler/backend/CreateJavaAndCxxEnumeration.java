...
public class CreateJavaAndCxxEnumeration extends DepthFirstAdapter {
    ...
    _trace.out( "inAOperation(...)" );
	 ...
	 public void outAEnumDeclarationPackageElement(AEnumDeclarationPackageElement node) {
	    ...
	    try {
	    	_templateFilesForJavaEnums.peek().addMapping("__CONSTUCTORS__",_constructors);
	    	_templateFilesForJavaEnums.peek().addMapping("__DEFAULT_CONSTUCTOR__",_defaultConstructor);
    ...
    try {
      ...
      TemplateFile templateForJava = new TemplateFile( _templateFilesForJavaEnums.peek(), javaTemplateFile );
      
      TemplateFile templateForCxx = new TemplateFile( _templateFilesForCxxEnums.peek(), cxxTemplateFile );
      String delim="";
      if(_multipleEnumerators)
      	delim=",";
      _constructors+="if(v=="+node.getName().getText()+".value())\n";
      ...
      _defaultConstructor="return "+node.getName().getText()+";\n";
      templateForJava.addMapping( "__ENUMERATOR_NAME__" ,delim+ node.getName().getText() );
      templateForJava.addMapping( "__ENUMERATOR_VALUE__" ,node.getValue().getText() );
      templateForCxx.addMapping( "__ENUMERATOR_NAME__" ,delim+ node.getName().getText() );
      templateForCxx.addMapping( "__ENUMERATOR_VALUE__" , node.getValue().getText() );
      templateForJava.open();
      templateForJava.close();
      templateForCxx.open();
      templateForCxx.close();
