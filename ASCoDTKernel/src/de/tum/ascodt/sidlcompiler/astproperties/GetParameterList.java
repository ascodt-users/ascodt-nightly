...
public class GetParameterList extends DepthFirstAdapter {
    ...
    for (Parameter parameter : _parameters) {
      ...
      if (!parameter.isArray && !parameter.isOut &&
        ...
        String fullQuualifiedTypeName = _scope
            .getFullQualifiedName(parameter.userDefinedTypeIdentifier);
      ...
      if (!parameter.isArray && parameter.isOut &&
          parameter.type != Parameter.Type.Boolean) {
        result += "[0]";
      }
  ...
  public String getParameterListInF(boolean areAllParametersInParameters) {
    ...
    for (Parameter parameter : _parameters) {
      ...
      // result += "std::string* " + parameter.name + "" +
      // ", const int "+parameter.name+"_len";
      // }
      // if(parameter.type==Parameter.Type.UserDefined && parameter.isArray){
      // AEnumDeclarationPackageElement
      // enumDecl=_scope.getEnumerationDefinition(parameter.userDefinedTypeIdentifier);
      // assert(enumDecl!=null);
      // String
      // fullQuualifiedTypeName=_scope.getFullQualifiedName(parameter.userDefinedTypeIdentifier);
      // result +=fullQuualifiedTypeName.replaceAll("[.]", "::")+"* " +
      // parameter.name + "" + ", const int "+parameter.name+"_len";
      // }
      // if (parameter.type==Parameter.Type.Opaque && parameter.isArray) {
    ...
    for (Parameter parameter : _parameters) {
      ...
      if (parameter.type == Parameter.Type.String && !parameter.isArray) {
        ...
        result += "sendData((char*)" + parameter.name + ".c_str()," +
            parameter.name + ".size(),_sendBuffer,_newsockfd,_buffer_size);\n";
      }
      if (parameter.type == Parameter.Type.UserDefined && !parameter.isArray) {}
      if (parameter.type == Parameter.Type.Opaque && !parameter.isArray) {
        result += "sendData((char*)&" + parameter.name +
