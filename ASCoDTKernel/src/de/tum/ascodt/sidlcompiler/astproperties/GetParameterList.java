...
public class GetParameterList extends DepthFirstAdapter {
	...
	public String pushInToSocketForCxx() {
		...
		for (Parameter parameter: _parameters) {
			...
			if (parameter.type==Parameter.Type.Boolean && (!parameter.isArray)) {
				result+="int "+parameter.name+"_as_int="+parameter.name+"?1:0;\n";
				result+="sendData((char*)&"+parameter.name+"_as_int,sizeof(int),_sendBuffer,_newsockfd,_buffer_size);\n";
			}
			if (parameter.type==Parameter.Type.Double && (!parameter.isArray )) {
			...
			if (parameter.type==Parameter.Type.UserDefined && (!parameter.isArray )) {
				...
				result+="sendData((char*)&"+parameter.name+"_enum2int,sizeof(int),_sendBuffer,_newsockfd,_buffer_size);\n";
			}
			if (parameter.type==Parameter.Type.Opaque && (!parameter.isArray)) {
...
//				result+="sendData((char*)"+parameter.name+".c_str(),sizeof("+parameter.name+".size()),_sendBuffer,_newsockfd,_buffer_size);\n";
			...
			if (parameter.type==Parameter.Type.UserDefined && (!parameter.isArray )) {
				result+="sendIntData("+parameter.name+".value());\n";
			}
			if (parameter.type==Parameter.Type.Opaque && (!parameter.isArray)) {
				result+="sendLongData("+parameter.name+");\n";
			}
  ...
  public String pullInFromSocketForJava(){
  	String result = "";
		for (Parameter parameter: _parameters) {
			...
			if (parameter.type==Parameter.Type.UserDefined && (!parameter.isArray ) && !parameter.isOut) {
				result+=parameter.userDefinedTypeIdentifier+" "+parameter.name+"="+parameter.userDefinedTypeIdentifier+".fromInt(readIntData());\n";
			}
			if (parameter.type==Parameter.Type.Opaque && (!parameter.isArray) && !parameter.isOut ) {
	...
	public String pullInFromSocketForCxx(){
		...
		for (Parameter parameter: _parameters) {
			...
			if (parameter.type==Parameter.Type.UserDefined && (!parameter.isArray )) {
				...
				result+="readData((char*)&"+parameter.name+"_int2enum,sizeof(int),rcvBuffer,newsockfd,buffer_size);\n";
				result +=parameter.name+"=("+fullQuualifiedTypeName.replaceAll("[.]", "::")+")"+parameter.name+"_int2enum;\n";
			}
			if (parameter.type==Parameter.Type.Opaque && (!parameter.isArray)) {
				result+="int "+parameter.name+";\n";
	...
	public String pushOutToSocketForCxx(){
		...
		for (Parameter parameter: _parameters) {
				...
				if (parameter.isOut&&parameter.type==Parameter.Type.UserDefined && (!parameter.isArray )) {
					...
					result+="sendData((char*)&"+parameter.name+"_int2enum,sizeof(int),sendBuffer,newsockfd,buffer_size);\n";
				}
	...
	public String pushOutToSocketForJava(){
		...
		for (Parameter parameter: _parameters) {
			if(parameter.isOut){
				...
				if (parameter.type==Parameter.Type.UserDefined && (!parameter.isArray )) {
					result+="int "+parameter.name+"_enum2int="+parameter.name+";\n";
					result+="sendData((char*)&"+parameter.name+"_enum2int,sizeof(int),sendBuffer,newsockfd,buffer_size);\n";
				}
				if (parameter.type==Parameter.Type.Opaque && (!parameter.isArray)) {
					result+="sendData((char*)&"+parameter.name+",sizeof(long long),sendBuffer,newsockfd,buffer_size);\n";
				}
		...
		for (Parameter parameter: _parameters) {
			if(parameter.isOut){
				...
				if (parameter.type==Parameter.Type.Boolean && (!parameter.isArray)) {
					...
					result+="readData((char*)&"+parameter.name+"_as_int,sizeof(int),_rcvBuffer,_newsockfd,_buffer_size);\n";
				...
				if (parameter.type==Parameter.Type.UserDefined && (!parameter.isArray )) {
					result+="int "+parameter.name+"_int2enum;\n";
					result+="readData((char*)&"+parameter.name+"_int2enum,sizeof(int),_rcvBuffer,_newsockfd,_buffer_size);\n";
					AEnumDeclarationPackageElement enumDecl=_scope.getEnumerationDefinition(parameter.userDefinedTypeIdentifier);
					assert(enumDecl!=null);
					String fullQuualifiedTypeName=_scope.getFullQualifiedName(parameter.userDefinedTypeIdentifier);
