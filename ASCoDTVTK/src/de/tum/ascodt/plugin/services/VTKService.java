 ...
 * This service is access point to the vtk view. Clients can add, remove, turn on/off pipelines
...
public class VTKService {
	...
	private VTKView getVTKView() throws ASCoDTException{
		...
		throw new ASCoDTException(getClass().getName(), "getVTKView()", "calling getVTKView() with non valid eclipse setup", null);
	}
	...
	private boolean isValid(){
		...
		if(_view==null){
			...
			class CustomRunnable implements Runnable{
				...
				@Override
				public void run() {
					_valid=!(PlatformUI.getWorkbench()==null||PlatformUI.getWorkbench().getActiveWorkbenchWindow()==null
	 ...
	 * @param vtkProp actor of the pipeline
	 */
		...
		PlatformUI.getWorkbench().getDisplay().asyncExec(new Runnable(){
			...
			public void run() {
				try {
					...
					ErrorWriterDevice.getInstance().showError( getClass().getName(), "getVTKView()", "vtk view cannot be initialized", e);
				} catch (InterruptedException e) {
					ErrorWriterDevice.getInstance().showError( getClass().getName(), "addPipeline()",  "vtk view waitng thread interrupted", e );
				} catch (ASCoDTException e) {
					ErrorWriterDevice.getInstance().showError( getClass().getName(), "addPipeline()",  "Cannot add new pipeline", e );
				}
			}
	...
	public vtkRenderWindowInteractor getInteractor() throws PartInitException, InterruptedException, ASCoDTException{
		initView();
		return getVTKView().getInteractor();
	}

	public void addObserver(final String event,final Object target, final String method){
		PlatformUI.getWorkbench().getDisplay().syncExec(new Runnable(){
			...
			public void run() {
				try {
					...
					ErrorWriterDevice.getInstance().showError( getClass().getName(), "removePipeline()",  "Cannot remove the vtk pipeline from the view", e );
				}
			}
	...
	private void initView() throws InterruptedException, PartInitException {
		...
		if(_view==null)
			setView(
					(VTKView) PlatformUI.getWorkbench().getActiveWorkbenchWindow().getActivePage().showView(VTKView.ID)
			);
	}
