...
DWORD WINAPI Foo_dispatch_loop(void *arg){
     ((test::FooNativeSocketDispatcher*) arg)->dispatch();
}
#else
...
test::FooNativeSocketDispatcher::FooNativeSocketDispatcher(char* host,int port,int bufferSize):
     _buffer_size(bufferSize){
     _rcvBuffer=new char[_buffer_size];
     _sendBuffer=new char[_buffer_size];
     
     test::FooNativeSocketDispatcher::open(host,port,_sockfd,_newsockfd);
     #ifdef _WIN32
...
test::FooNativeSocketDispatcher::FooNativeSocketDispatcher(int port,int bufferSize):
      _buffer_size(bufferSize){
     _rcvBuffer=new char[_buffer_size];
     _sendBuffer=new char[_buffer_size];
     //test::FooNativeSocketDispatcher::open(port,_sockfd,_newsockfd);
      #ifdef _WIN32
     CreateThread(NULL, 0, Foo_dispatch_loop, this, 0, NULL);
     #else
     pthread_t task;
...
test::FooNativeSocketDispatcher::~FooNativeSocketDispatcher(){
     delete [] _rcvBuffer;
     delete [] _sendBuffer;
     test::FooNativeSocketDispatcher::close(_sockfd,_newsockfd);
     
}


void test::FooNativeSocketDispatcher::open(char* hostname,int port,
#ifdef _WIN32
    SOCKET
#else
    int
...
/*
void test::FooNativeSocketDispatcher::open(int port,int &sockfd,int &newsockfd){
          socklen_t clilen;
...
void test::FooNativeSocketDispatcher::sendData(char* data, size_t numberOfBytes, char* sendBuffer,
#ifdef _WIN32
    SOCKET
#else
    int
...
void test::FooNativeSocketDispatcher::readData(char* data,size_t size_of_data,char* readBuffer,
#ifdef _WIN32
    SOCKET
#else
    int
...
void test::FooNativeSocketDispatcher::close(
#ifdef _WIN32
    SOCKET
#else
    int
...
void connect_Foo(void* ref,test::Foo* port){
    
     ((test::FooNativeSocketDispatcher*)ref)->connect(port);
}

void disconnect_Foo(void* ref,test::Foo* port){
     ((test::FooNativeSocketDispatcher*)ref)->disconnect(port);
     delete port;
}

void test::FooNativeSocketDispatcher::dispatch(){
     int methodId=0;
     long long ref;
     void (*invokers[2])(void*,test::Foo*);
     ...
     do{
          ...
          if(methodId!=-1){
               ...
               (*invokers[methodId])(this,(test::Foo*)ref);
          }
     }while(methodId!=-1);
}
