...
void VTKStencil::apply ( FlowField & flowField, int i, int j, int k ) {
	...
	cellIdStreamX << flowField.getCellIdsX().getValue(i,j,k)<<std::endl;
	cellIdStreamY << flowField.getCellIdsY().getValue(i,j,k)<<std::endl;
	cellIdStreamZ << flowField.getCellIdsZ().getValue(i,j,k)<<std::endl;
}
...
void VTKStencil::write ( FlowField & flowField ){
	...
	if (flowField.getDim() == 3){
		...
		_ofile << pressureStream.str() << std::endl;
		pressureStream.str("");
		_ofile<< "SCALARS cellidx float 1" << std::endl
				<< "LOOKUP_TABLE default" << std::endl;
		_ofile << cellIdStreamX.str() << std::endl;
		cellIdStreamX.str("");
		_ofile<< "SCALARS cellidy float 1" << std::endl
				<< "LOOKUP_TABLE default" << std::endl;
		_ofile << cellIdStreamY.str() << std::endl;
		cellIdStreamY.str("");
		_ofile<< "SCALARS cellidz float 1" << std::endl
				<< "LOOKUP_TABLE default" << std::endl;
		_ofile << cellIdStreamZ.str() << std::endl;
		cellIdStreamZ.str("");
		// Write velocity
		// _ofile << "CELL_DATA " << flowField.getNx() * flowField.getNy() << std::endl
		_ofile << "VECTORS velocity float" << std::endl;
