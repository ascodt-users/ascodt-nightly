...
inline void loadLocalVelocity2D(FlowField & flowField, FLOAT * localVelocity, int i, int j){
    for (int row = -1; row <= 1; row++ ){
        for ( int column = -1; column <= 1; column ++ ){
            FLOAT * point = flowField.getVelocity().getVector(i + column, j + row);
...
void loadLocalVelocity3D(FlowField & flowField, FLOAT * localVelocity, int i, int j, int k){
    for ( int layer = -1; layer <= 1; layer ++ ){
        for ( int row = -1; row <= 1; row++ ){
            for ( int column = -1; column <= 1; column ++ ){
                FLOAT * point = flowField.getVelocity().getVector(i + column, j + row, k + layer);
...
inline FLOAT computeF2D(const FLOAT * localVelocity, const Parameters & parameters, FLOAT dt){
...
inline FLOAT computeG2D(const FLOAT * localVelocity, const Parameters & parameters, FLOAT dt){
...
inline FLOAT computeF3D(const FLOAT * localVelocity, const Parameters & parameters, FLOAT dt){
    return localVelocity [mapd(0,0,0,0)]
                +  dt * ( 1 / parameters.flow.Re * ( d2udx2 ( localVelocity, parameters )
...
inline FLOAT computeG3D(const FLOAT * localVelocity, const Parameters & parameters, FLOAT dt){
    return localVelocity [mapd(0,0,0,1)]
                +  dt * ( 1 / parameters.flow.Re * ( d2vdx2 ( localVelocity, parameters )
...
inline FLOAT computeH3D(const FLOAT * localVelocity, const Parameters & parameters, FLOAT dt){
