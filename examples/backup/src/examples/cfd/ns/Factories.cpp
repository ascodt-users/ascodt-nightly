...
GlobalBoundaryFactory::GlobalBoundaryFactory(Parameters & parameters):
    ...
    _channelInput[0] = new ChannelInputVelocityStencil(parameters);
    _channelInput[1] = new ChannelInputFGHStencil(parameters);

    _BFChannelInput[0] = new BFInputVelocityStencil(parameters);
    _BFChannelInput[1] = new BFInputFGHStencil(parameters);
    ...
    // Let periodic boundaries override previous conditions
    if (parameters.walls.periodicX){
        _velocityStencils[0] = _periodic[0];
        _velocityStencils[1] = _periodic[0];
        _FGHStencils[0] = _periodic[1];
        _FGHStencils[1] = _periodic[1];
    }

    if (parameters.walls.periodicY){
        _velocityStencils[2] = _periodic[0];
        _velocityStencils[3] = _periodic[0];
        _FGHStencils[2] = _periodic[1];
        _FGHStencils[3] = _periodic[1];
    }

    if (parameters.walls.periodicZ){
        _velocityStencils[4] = _periodic[0];
        _velocityStencils[5] = _periodic[0];
        _FGHStencils[4] = _periodic[1];
        _FGHStencils[5] = _periodic[1];
    }
}
