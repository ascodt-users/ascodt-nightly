...
private:
	...
	const int _dim;    //! Dimensionality of the field
	...
	IntScalarField _cellIdsX; //! Scalar field representing the pressure
	IntScalarField _cellIdsY; //! Scalar field representing the pressure
	IntScalarField _cellIdsZ; //! Scalar field representing the pressure

	VectorField _FGH;

	ScalarField _RHS;      //! Right hand side for the Poisson equation

	FLOAT _localVelocity [3];   //! Array to store velocity on a single position
...
public:
	...
	/** Copy state of the field to another field
	 */
	void copyToField(FlowField & dest);

	/** Interpolates into this field using past and future fields, and a time parameter
	 */
	void interpolate(FlowField & past, FlowField & future, FLOAT t);
	 ...
	 * @return Reference to flag field
	 */
	IntScalarField & getCellIdsX ();
	IntScalarField & getCellIdsY ();
	IntScalarField & getCellIdsZ ();
	...
	void getPressureAndVelocity(FLOAT &pressure, FLOAT* &velocity, int i, int j);
