...
static FLOAT computeVelocity2D (FlowField & flowField, int i, int j, int stepSize,
...
static FLOAT computeVelocity3D (FlowField & flowField, int i, int j, int k, int stepSize,
                         const Parameters & parameters){
    if (j + parameters.parallel.firstCorner[1] >= stepSize + 2) {
        const int inletYSize = parameters.geometry.sizeY - stepSize;
