...
PetscUserCtx::PetscUserCtx(Parameters & parameters, FlowField & flowField):
    ...
    couplingLowerX (parameters.coupling.offsetNS[0] +
                    getOverlapWidth (parameters)),
    couplingUpperX (parameters.coupling.offsetNS[0] + parameters.coupling.sizeNS[0] - 1 -
                    getOverlapWidth (parameters)),
    couplingLowerY (parameters.coupling.offsetNS[1] +
                    getOverlapWidth (parameters)),
    couplingUpperY (parameters.coupling.offsetNS[1] + parameters.coupling.sizeNS[1] - 1 -
                    getOverlapWidth (parameters)),
    couplingLowerZ (parameters.coupling.offsetNS[2] +
                    getOverlapWidth (parameters)),
    couplingUpperZ (parameters.coupling.offsetNS[2] + parameters.coupling.sizeNS[2] - 1 -
                     getOverlapWidth (parameters))
{}
    ...
    LinearSolver(flowField, parameters), _ctx(parameters, flowField)
{
    ...
    const int Nx = parameters.geometry.sizeX + 2;
    const int Ny = parameters.geometry.sizeY + 2;
    const int Nz = parameters.geometry.sizeZ + 2;
    ...
    for (k = limitsZ[0]; k < limitsZ[1]; k++){
        for (j = limitsY[0]; j < limitsY[1]; j++){
            for (i = limitsX[0]; i < limitsX[1]; i++){
                ...
                // We will check here if the cell is in the boundaries or the inside of the
                // hole resulting from the coupling

                // We add 1 to the indexes, because these cells still refer to the coordinates with
                // two lower ghost cell layers
                if (parameters.coupling.set &&
                    (((i+1 == context->couplingLowerX || i+1 == context->couplingUpperX) &&
                       j+1 >= context->couplingLowerY && j+1 <= context->couplingUpperY &&
                       k+1 >= context->couplingLowerZ && k+1 <= context->couplingUpperZ)
                     ||
                     ((j+1 == context->couplingLowerY || j+1 == context->couplingUpperY) &&
                       i+1 >= context->couplingLowerX && i+1 <= context->couplingUpperX &&
                       k+1 >= context->couplingLowerZ && k+1 <= context->couplingUpperZ)
                    ...
                    // If the thing is in the cells that follow the Neumann boundary conditions.
                    int counter = 0;

                    // Check the six faces. Define Neumann boundary conditions on them
                    if (i+1 == context->couplingLowerX) {
                        stencilValues[counter] = 1.0;
                        column[counter].i = i-1; column[counter].j = j; column[counter].k = k;
                        counter ++;
                    }
                    if (i+1 == context->couplingUpperX) {
                        stencilValues[counter] = 1.0;
                        column[counter].i = i+1; column[counter].j = j; column[counter].k = k;
                        counter ++;
                    }
                    if (j+1 == context->couplingLowerY) {
                        stencilValues[counter] = 1.0;
                        column[counter].i = i; column[counter].j = j-1; column[counter].k = k;
                        counter ++;
                    }
                    if (j+1 == context->couplingUpperY) {
                        stencilValues[counter] = 1.0;
                        column[counter].i = i; column[counter].j = j+1; column[counter].k = k;
                        counter ++;
                    }
                    if (k+1 == context->couplingLowerZ) {
                        stencilValues[counter] = 1.0;
                        column[counter].i = i; column[counter].j = j; column[counter].k = k-1;
                        counter ++;
                    }
                    if (k+1 == context->couplingUpperZ) {
                        stencilValues[counter] = 1.0;
                        column[counter].i = i; column[counter].j = j; column[counter].k = k+1;
                        counter ++;
                    }
                    ...
                    MatSetValuesStencil(A, 1, &row, counter + 1, column, stencilValues,
                            INSERT_VALUES);
                } else { // A cell not in the coupling region
...
PetscErrorCode computeRHS3D(KSP ksp, Vec b, void* ctx){
    ...
    // Fill the inner nodes. We already have the values
    for (k = limitsZ[0]; k < limitsZ[1]; k++){
        for (j = limitsY[0]; j < limitsY[1]; j++){
            for (i = limitsX[0]; i < limitsX[1]; i++){
                ...
                if (parameters.coupling.set &&
                    ...
                    j+1 >= context->couplingLowerY && j+1 <= context->couplingUpperY &&
                    k+1 >= context->couplingLowerZ && k+1 <= context->couplingUpperZ){
                        array[k][j][i] = 0.0;   // For coupling. We use a zero for both B. Conds.
                } else {
                    const int obstacle = flags.getValue(i-limitsX[0]+2, j-limitsY[0]+2, k-limitsZ[0]+2);
                    if ((obstacle & OBSTACLE_SELF) == 0) {    // If this is a fluid cell
...
void PetscSolver::getLimits(int ** limitsX, int ** limitsY, int ** limitsZ){
     *limitsX = _limitsX;
     *limitsY = _limitsY;
     *limitsZ = _limitsZ;
}
