...
void NSLBCouplingStencil::loadVelocity(int component){

	//	for (int l = 0; l < stencilSize; l++){
	//        _values[l] = _velocities[component][_velocityIndex[component]*stencilSize+l];
	//
	//    }
	//	_velocityIndex[component]++;
}

void NSLBCouplingStencil::setVelocities(const  double* velocityX,const  double* velocityY,const double* velocityZ,const int velocitySize){
	_velocities[0].resize(velocitySize);
	_velocities[1].resize(velocitySize);
	_velocities[2].resize(velocitySize);
	for(int i=0;i<velocitySize;i++){
		_velocities[0][i]=velocityX[i];

		_velocities[1][i]=velocityY[i];

		_velocities[2][i]=velocityZ[i];

	}
}
void NSLBCouplingStencil::setJacobian(const double* jacobian,const int gradients_len){
	_jacobian.resize(gradients_len);
	for(int i=0;i<gradients_len;i++)
		_jacobian[i]=jacobian[i];

}
void NSLBCouplingStencil::setPressure(const double* pressure,const int velocitySize){
	_pressure.resize(velocitySize);
	for(int i=0;i<velocitySize;i++)
		_pressure[i]=pressure[i];
}

void NSLBCouplingStencil::loadPressure(){
	//    for (int l = 0; l < stencilSize; l++){
	//        _values[l] = _pressure[_pressureIndex*stencilSize+l];
	//    }
	//    _pressureIndex++;
}
...
void NSLBCouplingStencil::reset(){
	_velocityIndex=0;
	_pressureIndex=0;
}
...
void NSLBCouplingStencil::interpolate (int i, int j, int k, const int * const flip){
	...
	density=_pressure[_pressureIndex++];

	velocity[0]=_velocities[0][_velocityIndex];
	velocity[1]=_velocities[1][_velocityIndex];
	velocity[2]=_velocities[2][_velocityIndex];
	for(int i=0;i<9;i++){
		jacobian[i]=_jacobian[_velocityIndex*9+i];
	}
	_velocityIndex++;
	FLOAT eq[Q], neq[Q];
