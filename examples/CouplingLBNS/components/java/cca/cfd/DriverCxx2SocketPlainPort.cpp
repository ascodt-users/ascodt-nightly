...
cca::cfd::DriverCxx2SocketPlainPort::DriverCxx2SocketPlainPort(char* host,int port,int buffer_size):
     _buffer_size(buffer_size){
     _rcvBuffer=new char[_buffer_size];
     _sendBuffer=new char[_buffer_size];
     cca::cfd::DriverCxx2SocketPlainPort::open_client(host,port,_sockfd,_newsockfd);
     
}

cca::cfd::DriverCxx2SocketPlainPort::DriverCxx2SocketPlainPort(int port,int buffer_size):
    _buffer_size(buffer_size){
    _rcvBuffer=new char[_buffer_size];
    _sendBuffer=new char[_buffer_size];
    //cca::cfd::DriverCxx2SocketPlainPort::open_server(port,_sockfd,_newsockfd);
}

cca::cfd::DriverCxx2SocketPlainPort::~DriverCxx2SocketPlainPort(){
     int method=-1;
     ...
     cca::cfd::DriverCxx2SocketPlainPort::close(_sockfd,_newsockfd);
     
}


//int cca::cfd::DriverCxx2SocketPlainPort::getSockfd(){
//     return _sockfd;
//}
//int cca::cfd::DriverCxx2SocketPlainPort::getNewsockfd(){
//     return _newsockfd;
//}

void cca::cfd::DriverCxx2SocketPlainPort::open_client(char* hostname,int port,
#ifdef _WIN32
SOCKET
#else
int
...
/*
void cca::cfd::DriverCxx2SocketPlainPort::open_server(int port,int &sockfd,int &newsockfd){
          socklen_t clilen;
...
void cca::cfd::DriverCxx2SocketPlainPort::sendData(char* data, size_t numberOfBytes, char* sendBuffer,
#ifdef _WIN32
SOCKET
#else
int
...
void cca::cfd::DriverCxx2SocketPlainPort::readData(char* data,size_t size_of_data,char* readBuffer,
#ifdef _WIN32
SOCKET
#else
int
...
void cca::cfd::DriverCxx2SocketPlainPort::close(
#ifdef _WIN32
SOCKET
#else
int
...
void cca::cfd::DriverCxx2SocketPlainPort::go(const std::string configFile){
...
fcntl(_newsockfd, F_SETFL, flags);
     ...
     int configFile_data_size=configFile.size()+1;
sendData((char*)&configFile_data_size,sizeof(int),_sendBuffer,_newsockfd,_buffer_size);
sendData((char*)configFile.c_str(),configFile.size()+1,_sendBuffer,_newsockfd,_buffer_size);

     
}

void cca::cfd::DriverCxx2SocketPlainPort::goParallel(const std::string configFile){
     //assert(_destination!=NULL);
#ifdef _WIN32
#else
     int flags;
