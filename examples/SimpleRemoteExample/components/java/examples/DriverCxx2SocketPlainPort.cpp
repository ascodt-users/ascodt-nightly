...
examples::DriverCxx2SocketPlainPort::DriverCxx2SocketPlainPort(char* host,int port,int buffer_size):
     _buffer_size(buffer_size){
     _rcvBuffer=new char[_buffer_size];
     _sendBuffer=new char[_buffer_size];
     examples::DriverCxx2SocketPlainPort::open_client(host,port,_sockfd,_newsockfd);
     
}

examples::DriverCxx2SocketPlainPort::DriverCxx2SocketPlainPort(int port,int buffer_size):
    _buffer_size(buffer_size){
    _rcvBuffer=new char[_buffer_size];
    _sendBuffer=new char[_buffer_size];
    //examples::DriverCxx2SocketPlainPort::open_server(port,_sockfd,_newsockfd);
}

examples::DriverCxx2SocketPlainPort::~DriverCxx2SocketPlainPort(){
     int method=-1;
     ...
     examples::DriverCxx2SocketPlainPort::close(_sockfd,_newsockfd);
     
}


//int examples::DriverCxx2SocketPlainPort::getSockfd(){
//     return _sockfd;
//}
//int examples::DriverCxx2SocketPlainPort::getNewsockfd(){
//     return _newsockfd;
//}

void examples::DriverCxx2SocketPlainPort::open_client(char* hostname,int port,
#ifdef _WIN32
SOCKET
#else
int
...
/*
void examples::DriverCxx2SocketPlainPort::open_server(int port,int &sockfd,int &newsockfd){
          socklen_t clilen;
...
void examples::DriverCxx2SocketPlainPort::sendData(char* data, size_t numberOfBytes, char* sendBuffer,
#ifdef _WIN32
SOCKET
#else
int
...
void examples::DriverCxx2SocketPlainPort::readData(char* data,size_t size_of_data,char* readBuffer,
#ifdef _WIN32
SOCKET
#else
int
...
void examples::DriverCxx2SocketPlainPort::close(
#ifdef _WIN32
SOCKET
#else
int
...
void examples::DriverCxx2SocketPlainPort::go(){
...
void examples::DriverCxx2SocketPlainPort::goParallel(){
     //assert(_destination!=NULL);
     int flags;
     flags = fcntl(_newsockfd, F_GETFL, 0);
     flags ^= O_NONBLOCK;
