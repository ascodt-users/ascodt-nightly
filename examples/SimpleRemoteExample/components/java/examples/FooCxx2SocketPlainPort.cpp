...
examples::FooCxx2SocketPlainPort::FooCxx2SocketPlainPort(char* host,int port,int buffer_size):
     _buffer_size(buffer_size){
     _rcvBuffer=new char[_buffer_size];
     _sendBuffer=new char[_buffer_size];
     examples::FooCxx2SocketPlainPort::open_client(host,port,_sockfd,_newsockfd);
     
}

examples::FooCxx2SocketPlainPort::FooCxx2SocketPlainPort(int port,int buffer_size):
    _buffer_size(buffer_size){
    _rcvBuffer=new char[_buffer_size];
    _sendBuffer=new char[_buffer_size];
    //examples::FooCxx2SocketPlainPort::open_server(port,_sockfd,_newsockfd);
}

examples::FooCxx2SocketPlainPort::~FooCxx2SocketPlainPort(){
     int method=-1;
     ...
     examples::FooCxx2SocketPlainPort::close(_sockfd,_newsockfd);
     
}


//int examples::FooCxx2SocketPlainPort::getSockfd(){
//     return _sockfd;
//}
//int examples::FooCxx2SocketPlainPort::getNewsockfd(){
//     return _newsockfd;
//}

void examples::FooCxx2SocketPlainPort::open_client(char* hostname,int port,
#ifdef _WIN32
SOCKET
#else
int
...
/*
void examples::FooCxx2SocketPlainPort::open_server(int port,int &sockfd,int &newsockfd){
          socklen_t clilen;
...
void examples::FooCxx2SocketPlainPort::sendData(char* data, size_t numberOfBytes, char* sendBuffer,
#ifdef _WIN32
SOCKET
#else
int
...
void examples::FooCxx2SocketPlainPort::readData(char* data,size_t size_of_data,char* readBuffer,
#ifdef _WIN32
SOCKET
#else
int
...
void examples::FooCxx2SocketPlainPort::close(
#ifdef _WIN32
SOCKET
#else
int
...
void examples::FooCxx2SocketPlainPort::bar(const int a,const int b){
...
fcntl(_newsockfd, F_SETFL, flags);
     ...
     sendData((char*)&a,sizeof(int),_sendBuffer,_newsockfd,_buffer_size);
sendData((char*)&b,sizeof(int),_sendBuffer,_newsockfd,_buffer_size);

     
}

void examples::FooCxx2SocketPlainPort::barParallel(const int a,const int b){
     //assert(_destination!=NULL);
     int flags;
     flags = fcntl(_newsockfd, F_GETFL, 0);
     flags ^= O_NONBLOCK;
