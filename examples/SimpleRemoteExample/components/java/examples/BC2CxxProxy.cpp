#include "examples/BC2CxxProxy.h"
#ifdef _WIN32
	#include <winsock2.h>
	#include <ws2tcpip.h>
...
void invoker_create_instance(void** ref,int,int,char*,char*
     ...
     *ref = new examples::BImplementation();
}
...
void invoker_destroy_instance(void** ref,int,int,char*,char*
	 ...
	 ,MPI_Comm communicator,int methodID
      ...
      delete ((examples::BImplementation*)*ref);
...
,MPI_Comm communicator, int methodId
  ...
  int a;
readData((char*)&a,sizeof(int),rcvBuffer,newsockfd,buffer_size);
int b;
readData((char*)&b,sizeof(int),rcvBuffer,newsockfd,buffer_size);

  ((examples::BImplementation*)*ref)->bar(a,b);
  
}


void parallel_master_invoker_bar(void** ref,int newsockfd, int buffer_size,char* rcvBuffer, char* sendBuffer
#ifdef Parallel
,MPI_Comm communicator, int methodId
#endif
){
...
readData((char*)&b,sizeof(int),rcvBuffer,newsockfd,buffer_size);
  ...
  broadcastParallelData((char*)&a,sizeof(int),communicator);
broadcastParallelData((char*)&b,sizeof(int),communicator);
...
void parallel_worker_invoker_bar(void** ref
#ifdef Parallel
,MPI_Comm newsockfd
#endif
){
  #ifdef Parallel
  int a;
broadcastParallelData((char*)&a,sizeof(int),newsockfd);
int b;
broadcastParallelData((char*)&b,sizeof(int),newsockfd);
...
std::string retrieveSocketAddress(){
	...
	assert(_sockfd>=0);
	const char* daemon_port = getenv("EXAMPLES_B_DAEMON_PORT");
	const char* network_interface = getenv("EXAMPLES_B_NET_INTERFACE");
...
,MPI_Comm communicator
     ...
     int methodId=0;
     invokers[11]=parallel_master_invoker_bar;
...
MPI_Comm clientfd){
     ...
     int methodId=0;
     parallel_worker_invokers[10]=parallel_worker_invoker_bar;
...
void* parallel_deamon_run(void* daemon_args){
	parallel_worker_loop(((EXAMPLES_B_arg*)daemon_args)->ref,((EXAMPLES_B_arg*)daemon_args)->communicator);
}
#endif
...
void* server_deamon_run(void* daemon_args){
      ...
      accept_on_server(((EXAMPLES_B_arg*)daemon_args)->daemon_serverfd,clientfd);
      std::cout<<"server accepted"<<std::endl;
      socket_worker_loop(
      	((EXAMPLES_B_arg*)daemon_args)->ref,
      	clientfd,
      	((EXAMPLES_B_arg*)daemon_args)->buffer_size
#ifdef Parallel
		,((EXAMPLES_B_arg*)daemon_args)->communicator
#endif      	
      	);
}
void startMPIDaemon(EXAMPLES_B_arg& arg){
#ifdef Parallel	 
int rank = -1;
MPI_Comm_rank(MPI_COMM_WORLD,&rank);
if(rank>0){
...
void startSocketDaemons(EXAMPLES_B_arg& arg){
...
#ifdef _WIN32
void INITIALISE(EXAMPLES_B_arg& arg){
#else
void initialise_(EXAMPLES_B_arg& arg){
#endif
   const char* client_port = getenv("EXAMPLES_B_PORT"); 
   const char* daemon_port = getenv("EXAMPLES_B_DAEMON_PORT");
   const char* buffer_size = getenv("EXAMPLES_B_BUFFER_SIZE");
   const char* hostname = getenv("EXAMPLES_B_HOSTNAME");
   const char* java_client_flag = getenv("EXAMPLES_B_JAVA");
   const char* number_of_workers = getenv("EXAMPLES_B_WORKERS"); 
...
#ifdef _WIN32
void DESTROY(EXAMPLES_B_arg& arg){
#else
void destroy_(EXAMPLES_B_arg& arg){
#endif
#ifdef _WIN32
...
#ifdef _WIN32
void SOCKET_LOOP(EXAMPLES_B_arg& arg){
#else
void socket_loop_(EXAMPLES_B_arg& arg){
#endif
  if(arg.java_client_flag)       
